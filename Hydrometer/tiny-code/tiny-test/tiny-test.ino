...
boolean found = false; 
int t_celsius; //make local later
int rawVcc;
...
esp8266.begin(9600);
 ...
 // sendCommand(F("AT+CWMODE=1"),5,"OK",true);
  //delay(2000);
  //sendCommand("AT+CWJAP=\""+ AP +"\",\""+ PASS +"\"",20,"OK",false);
...
getTempAndVcc();
sendData(t_celsius,F("field1"));
sendData(rawVcc, F("field2"));
updateAccelData();
...
while(countTimeCommand < (5*1))
 ...
 esp8266.println(String(4+API.length() + field.length()+22 + String(sensorValue).length()));///
 delay(5000);
...
while(countTimeCommand < (5*1))
  { esp8266.print(F("GET /update?api_key="));
    ...
    esp8266.print(field);
    esp8266.print(F("="));
    esp8266.println(String(sensorValue));
...
//multiply by 100 to allow us to store as int with float precision
  rawVcc *=100;
  t_celsius *= 100;
}
